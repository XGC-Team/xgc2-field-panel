...
感知（动捕 + `estimator_vrpn_ugv_state` 别名拉起 3D `Pose3InertialEskf`，订 `/imu/data_raw`）和控制（参考 / NMPC / 摆渡）是两组启停。估计话题是 `RigidStateEstimate`，面板 compare 用投影后的 x/y/yaw；`estimator_state=3` 才算 RUNNING。先看 IMU、CAN、动捕、估计是否新鲜，再启控制。默认 `FIELD_PANEL_STATE_SOURCE=estimator`。VRPN 直通是错误旁路。
